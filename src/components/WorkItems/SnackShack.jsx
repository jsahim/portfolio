...
const SnackShack = () => {
  ...
  return (
    ...
    <div ref={projectTop} className="text-black border-4 border-[#f22b40] bg-white rounded-lg p-10">
      ...
      <div className="space-y-4 grid md:grid-cols-2 gap-15 md:gap-8 pt-10">
        <div>
          IMAGE
        </div>
        <div>
      ...
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${drawerOpen ? 'max-h-[5000px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="list-decimal list-inside space-y-4 grid md:grid-cols-2 gap-15 md:gap-8">
          ...
          <div>
            <p>THE SOLUTION: <span className="text-gray-600 font-semibold">A MOBILE-FIRST PRE-ORDERING APP</span><br/>The solution was to provide patrons with easy, remote access to the full menu and the ability to place orders before they even approached the stand, addressing indecision and queue length simultaneously.The solution was to provide patrons with easy, remote access to the full menu and the ability to place orders before they even approached the stand, addressing indecision and queue length simultaneously.</p>
